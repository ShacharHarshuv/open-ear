...
export function chordsInRealSongsExercise(progressionList: DeepReadonly<ProgressionInSongFromYouTubeDescriptor[]> = chordsInRealSongsDescriptorList) {
  function getAvailableProgressions(settings: ChordsInRealSongsSettings): DeepReadonly<ProgressionInSongFromYouTubeDescriptor[]> {
    const isChordProgressionValid = (chords: DeepReadonly<ProgressionInSongFromYouTubeDescriptor>['chords']): boolean => {
      return _.every(chords, chord => settings.includedChords.includes(chord.chord));
    }
     ...
     * */
    const soloedProgressions: DeepReadonly<ProgressionInSongFromYouTubeDescriptor[]> = _.filter(progressionList, 'solo');
  if (!_.isEmpty(soloedProgressions)) {
    return soloedProgressions
  }

  const validChordProgressionsDescriptorList: DeepReadonly<ProgressionInSongFromYouTubeDescriptor[]> = progressionList
