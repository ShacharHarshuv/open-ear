...
export function isInKey(note: NoteNumberOrName, key: Key) {
  function transposeToFirstOctave(note: NoteNumberOrName): Note {
    return toNoteName(
      mod(toNoteNumber(note) - toNoteNumber('C1'), Interval.Octave) +
        toNoteNumber('C1')
    );
  }
  const noteTransposedToFirstOctave: Note = transposeToFirstOctave(note);
  const distanceOfKeyFromC: number = mod(
    getDistanceOfKeys(key, 'C'),
    Interval.Octave
  );
  ...
  return scaleOfKeyInFirstOctave
    .map(toNoteNumber)
    .includes(toNoteNumber(noteTransposedToFirstOctave));
}
