...
> = FnList extends [infer Fn3, ...infer FnListRest]
  ? Fn3 extends ComposableFunction
    ? FnListRest extends ComposableFunction[]
      ? ComposedParams<
          MergeMap,
          ComposedFunctionForTwoFns<MergeMap, Fn1, Fn2>,
          Fn3,
          FnListRest
        >
      : never
...
export type PropertiesMergeConfig<Params extends object> = {
  [Key in keyof Params]?: MergeFunction<Params[Key]>;
};
...
>(keyToMergeFn: MergeMap) {
  ...
  >(fn1: Fn1, fn2: Fn2, ...fnList: FnList) {
    function mergeValues(
      ...
      for (let key in value2) {
        if (value1[key] && keyToMergeFn[key]) {
           ...
           * */
          returnValue[key] = flipMergeOrder
            ? keyToMergeFn[key](value2[key], value1[key])
            : keyToMergeFn[key](value1[key], value2[key]);
        } else {
    ...
    return (
      ...
      return mergeValues(fn1Return, fn2Return) as ComposedReturn<
        MergeMap,
        Fn1,
        Fn2,
        FnList
